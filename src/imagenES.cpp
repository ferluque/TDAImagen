...
//-----------------------------------------------------------------
/*
Imagen ArrayToImagen (const byte* datos, const int filas, const int columnas) {
  Imagen nueva (filas, columnas);
  for (int i=0; i<nueva.num_filas(); i++)
    for (int j=0; j<nueva.num_columnas(); j++)
      nueva.asigna_pixel(i,j,datos[i*filas+j]);
  return nueva;
}

byte* ImagenToArray(const Imagen& imagen) {
  byte* array = new byte [imagen.num_filas()*imagen.num_columnas()];
  for (int i=0; i<imagen.num_filas(); i++)
    for (int j=0; j<imagen.num_columnas(); j++)
      array[i*imagen.num_filas()+j] = imagen.valor_pixel(i,j);
  return array;
}
/*
